from PIL import Image
import numpy as np

# DES constants
IP = [58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7]

FP = [40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31,
      38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29,
      36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27,
      34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25]

E = [32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13,
     12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23,
     24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1]

S_BOXES = [
    # S1
    [[14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
     [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
     [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
     [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]],
    # S2
    [[15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
     [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
     [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
     [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]],
    # S3
    [[10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
     [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
     [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
     [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]],
    # S4
    [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
     [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
     [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
     [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]],
    # S5
    [[2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
     [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
     [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
     [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]],
    # S6
    [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
     [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
     [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
     [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]],
    # S7
    [[4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
     [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
     [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
     [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]],
    # S8
    [[13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
     [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
     [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
     [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]]
]

P = [16, 7, 20, 21, 29, 12, 28, 17,
     1, 15, 23, 26, 5, 18, 31, 10,
     2, 8, 24, 14, 32, 27, 3, 9,
     19, 13, 30, 6, 22, 11, 4, 25]

PC1 = [57, 49, 41, 33, 25, 17, 9,
       1, 58, 50, 42, 34, 26, 18,
       10, 2, 59, 51, 43, 35, 27,
       19, 11, 3, 60, 52, 44, 36,
       63, 55, 47, 39, 31, 23, 15,
       7, 62, 54, 46, 38, 30, 22,
       14, 6, 61, 53, 45, 37, 29,
       21, 13, 5, 28, 20, 12, 4]

PC2 = [14, 17, 11, 24, 1, 5,
       3, 28, 15, 6, 21, 10,
       23, 19, 12, 4, 26, 8,
       16, 7, 27, 20, 13, 2,
       41, 52, 31, 37, 47, 55,
       30, 40, 51, 45, 33, 48,
       44, 49, 39, 56, 34, 53,
       46, 42, 50, 36, 29, 32]

SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2,
          1, 2, 2, 2, 2, 2, 2, 1]

def permute(block, table):
    return ''.join([block[x-1] for x in table])

def xor(block1, block2):
    return ''.join(['1' if bit1 != bit2 else '0' for bit1, bit2 in zip(block1, block2)])

def sbox_substitution(bits):
    output = ""
    for i in range(8):
        block = bits[i*6:(i+1)*6]
        row = int(block[0] + block[5], 2)
        col = int(block[1:5], 2)
        output += f"{S_BOXES[i][row][col]:04b}"
    return output

def feistel(right, subkey):
    expanded_right = permute(right, E)
    xored = xor(expanded_right, subkey)
    substituted = sbox_substitution(xored)
    return permute(substituted, P)

def generate_subkeys(key):
    permuted_key = permute(key, PC1)
    left, right = permuted_key[:28], permuted_key[28:]
    subkeys = []
    for shift in SHIFTS:
        left = left[shift:] + left[:shift]
        right = right[shift:] + right[:shift]
        subkeys.append(permute(left + right, PC2))
    return subkeys

def des_encrypt_block(block, subkeys):
    permuted_block = permute(block, IP)
    left, right = permuted_block[:32], permuted_block[32:]
    for subkey in subkeys:
        temp_right = right
        right = xor(left, feistel(right, subkey))
        left = temp_right
    combined_block = right + left
    return permute(combined_block, FP)

def des_decrypt_block(block, subkeys):
    return des_encrypt_block(block, subkeys[::-1])

def pad_data(data):
    pad_len = 8 - len(data) % 8
    return data + chr(pad_len) * pad_len

def unpad_data(data):
    pad_len = ord(data[-1])
    return data[:-pad_len]

def string_to_bits(s):
    return ''.join(f"{ord(c):08b}" for c in s)

def bits_to_string(b):
    return ''.join(chr(int(b[i:i+8], 2)) for i in range(0, len(b), 8))

class DES:
    def __init__(self, key):
        key = string_to_bits(key)
        self.subkeys = generate_subkeys(key)

    def encrypt(self, plaintext):
        plaintext = pad_data(plaintext)
        ciphertext = ""
        for i in range(0, len(plaintext), 8):
            block = string_to_bits(plaintext[i:i+8])
            ciphertext += bits_to_string(des_encrypt_block(block, self.subkeys))
        return ciphertext

    def decrypt(self, ciphertext):
        plaintext = ""
        for i in range(0, len(ciphertext), 8):
            block = string_to_bits(ciphertext[i:i+8])
            plaintext += bits_to_string(des_decrypt_block(block, self.subkeys))
        return unpad_data(plaintext)

class DESWithImages(DES):
    def encrypt_image(self, image_path, output_path):
        with open(image_path, "rb") as img_file:
            img_data = img_file.read()
        padded_data = pad_data(img_data.decode("latin1"))
        encrypted_data = self.encrypt(padded_data)
        with open(output_path, "wb") as encrypted_file:
            encrypted_file.write(encrypted_data.encode("latin1"))
        print("Encryption of the Image Successful")

    def decrypt_image(self, image_path, output_path):
        with open(image_path, "rb") as encrypted_file:
            encrypted_data = encrypted_file.read()
        decrypted_data = self.decrypt(encrypted_data.decode("latin1"))
        unpadded_data = unpad_data(decrypted_data)
        with open(output_path, "wb") as decrypted_file:
            decrypted_file.write(unpadded_data.encode("latin1"))
        print("Decryption of the Image Successful")

# Helper function to test encryption and decryption
def test_des():
    key = "mysecret"
    des = DES(key)
    plaintext = "HelloWorld"
    ciphertext = des.encrypt(plaintext)
    decrypted_text = des.decrypt(ciphertext)
    assert decrypted_text == plaintext
    print(f"Original: {plaintext}")
    print(f"Encrypted: {ciphertext}")
    print(f"Decrypted: {decrypted_text}")
    # with open("DESencrypted.txt", "w") as text_file:
    #     text_file.write(ciphertext)

# Example usage:
des = DESWithImages("mysecretkey")
des.encrypt_image("cat.jpg", "encrypted.jpeg")
des.decrypt_image("encrypted.jpeg", "decrypted.jpeg")

test_des()


